...
public class MailUtil {
	...
	private static final String _port = "465";	//端口号 SSL验证,服务器的提供商过滤了smtp的端口:25
	...
	public void sendMessage(String to, String subject, String messageText) throws MessagingException, UnsupportedEncodingException {
		...
		props.setProperty("mail.smtp.port", _port);				// 指定SMTP服务器端口号
		...
		System.out.println(this.fromMail);
	...
	/*public static void main(String[] args) {
		...
		String subject = "账户余额不足1";
		String messageText = "账户余额不足2";
		try {
            MailUtil mu = new MailUtil("gnw@gwchina.cn", "Lwtx0123");
            mu.sendMessage(to, subject, messageText);
        } catch (MessagingException e) {
            e.printStackTrace();
        } catch (UnsupportedEncodingException e) {
